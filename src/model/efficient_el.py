...
class EfficientEL(LightningModule):
    ...
    def _tokens_scores_to_spans(self, batch, start, end, tokens, scores_el):
        ...
        # orignial code: eliminating overlapped spans, based on the mention length
        # for spans_ in spans:
        #     for e in [
        #         [x, y]
        #         for x in spans_
        ...
        for i in range(len(spans)):
            spans[i].sort(key=lambda x:x[1]-x[0]) # rank spans by length, long mention last
            new_spans = []
            while spans[i]:
                x = spans[i].pop() # pop out the longest mention
                overlapped = False
                for y in new_spans:
